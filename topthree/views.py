...
def select_topic(request):
    ...
    return render(request, 'topthree/select_topic.html', {'topics': topics})


@login_required
def select_topic_quantity(request, topic_id):
    topic = get_object_or_404(Topic, id=topic_id)
    return render(request, 'topthree/select_topic_quantity.html', {'topic': topic})
...
def start_topic_quiz(request, topic_id, num_questions):
    ...
    return render(request, 'topthree/start_topic_quiz.html', {'topics': topics, 'questions': questions, 'choices': choices})
