...
def select_semester(request):
    ...
    return render(request, 'modthree/select_semester.html', {'semesters': semesters})


@login_required
def select_semester_quantity(request, semester_id):
    semesters = get_object_or_404(Semester, id=semester_id)
    return render(request, 'modthree/select_semester_quantity.html', {'semesters': semesters})
...
def start_semester_quiz(request, semester_id, num_questions):
    ...
    return render(request, 'modthree/start_semester_quiz.html', {'semesters': semesters, 'questions': questions, 'choices': choices})
