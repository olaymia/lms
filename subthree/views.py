...
from .models import Subject, Question, Choice, UserResponseCliSub

@login_required
def select_subject(request):
    subjects = Subject.objects.all()
    return render(request, 'subthree/select_subject.html', {'subjects': subjects})
...
def select_quantity(request, subject_id):
    ...
    if request.method == 'POST':
        ...
        if num_questions < 1:
            ...
            return render(request, 'subthree/select_quantity.html', {'subject': subject})
...
def complete_quiz(request):
    ...
    return render(request, 'subthree/quiz_complete.html', {
